...
/**
 * Lit le levier effectif depuis gb.data.whatstrat.LEVERAGE.
 * L'objet whatstrat est toujours présent dans le Ledger Gunbot.
...
function getLeverage(gb) {
    try {
        const lev = parseFloat(gb.data.whatstrat.LEVERAGE);
        if (!isNaN(lev) && lev > 0) {
            log('getLeverage : LEVERAGE = ' + lev);
            return lev;
        }
        log('getLeverage : LEVERAGE absent, nul ou = 0 → levier = 1');
        return 1;
    } catch (e) {
        log('getLeverage : exception = ' + e.message + ' → levier = 1');
        return 1;
    }
}
...
module.exports = function (gb) {
    ...
    log('=== futureGbSimTools v0-2-0 initialisé ===');
